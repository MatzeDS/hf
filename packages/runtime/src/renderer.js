...
import { flushPostFlushCallbacks, queuePostFlushCallback } from "./scheduler.js";
import {
    EMPTY_OBJ,
    EMPTY_ARR,
    isArray,
...
export function unmount(vNode, parentComponent) {
    ...
    if (vNode.props.onVNodeBeforeUnmount) {
        vNode.props.onVNodeBeforeUnmount(vNode);
    }
    ...
    if (vNode.props.onVNodeUnmounted) {
        queuePostFlushCallback(() => {
            vNode.props.onVNodeUnmounted(vNode);
        });
    }
}
...
function patchComponent(oldNode, newNode, parentComponent) {
    ...
    instance.updateVNode(newNode, shouldUpdateComponent(oldNode, newNode));
}
...
function createComponentInstance(vNode, parent) {
    ...
    return new vNode.component(vNode.props, vNode, parent);
}
...
function mountElement(vNode, container, anchor, parentComponent, isSVG) {
    ...
    if (props) {
        ...
        if (props.onVNodeBeforeMount) {
            props.onVNodeBeforeMount(vNode);
        }
    }
    ...
    if (transition || props.onVNodeMounted) {
        queuePostFlushCallback(() => {
            props.onVNodeMounted?.(vNode);
            transition?.enter(el);
        });
    }
}
...
function patchElement(oldNode, newNode, parentComponent, isSVG) {
    ...
    parentComponent?.toggleRecurse(false);

    if (newProps.onVNodeBeforeUpdate) {
        newProps.onVNodeBeforeUpdate(newNode, oldNode);
    }

    parentComponent?.toggleRecurse(true);

    patchChildren(oldNode, newNode, el, null, parentComponent, isSVG);
    patchProps(el, oldProps, newProps, isSVG);

    if (newProps.onVNodeUpdated) {
        queuePostFlushCallback(() => {
            newProps.onVNodeUpdated(newNode, oldNode);
        });
    }
}
